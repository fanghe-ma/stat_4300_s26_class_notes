...
\begin{result}[Moments of Binomial random variables]
    ...
    \begin{align*}
        ...
        &= \sum\limits_{i = 1}^{n} \mathbb{E}\left[ X_i \right]  \quad \text{by linearity of expectation} \\
        &= \sum\limits_{i = 1}^{n}  p \\
        &= np
    \end{align*}
...
\begin{remark}[Concentration of binomial] 
    ...
    Take $t = 3 \sigma, \sigma = \sqrt{Var(S_n)}$. 
    \begin{align*}
        P \left(  \left| S_n - np \right|  \geq 3 \sigma \right)  &\leq \frac{Var(S_n)}{9 \sigma^2}= \frac{1}{9} \\
        P \left(  \left| S_n - np \right|  \leq 3 \sigma \right)  &\geq 1 - \frac{1}{9} \approx 88\%
    \end{align*}
    \textit{For symmetric binomial, we have 88\% chance of observing a value within 3 standard deviations of its mean.}
\end{remark}
