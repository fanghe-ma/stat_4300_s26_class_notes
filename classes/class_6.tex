...
\begin{result}[Geometric survival function]
    The survival function of a geometric random variable $T \sim \operatorname{Geom} (p)$ is sometimes denoted as 
    \[
        S(n) = P(T > n)  = 1 - F_X(n) = (1-p)^n \text{ for } n \geq 0
    \] 
\end{result}
...
\begin{proof}
    ...
    \begin{align*}
        \{ T > n \} &= \{ X_1 = 0, X_2 = 0, \hdots X_n = 0 \}  \\
        &= \bigcap_{i = 1}^n \{ X_i = 0 \} \\
        P \left( T > n \right) &= P \left( \bigcap_{i = 1}^n X_i = 0 \right)  \\
        &= \prod_{i = 1}^n P(X_i = 0) \text{ by independence }\\
        &= (1 - p)^n
    \end{align*}
\end{proof}
...
\begin{remark}[Concentration / Anti-concentration of $T$]
    When $p$ is small, the geometric random variable is less concetrated. Vice versa.
